...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("UPDATE `record` SET `eid`= ?,`aid`= ?,`uid`= ? WHERE eid = ? AND uid = ?;");

    $stmt->bindParam(1, $eid, PDO::PARAM_STR);
    $stmt->bindParam(2, $aid, PDO::PARAM_STR);
    $stmt->bindParam(3, $_SESSION['uid'], PDO::PARAM_STR);
    $stmt->bindParam(4, $eid, PDO::PARAM_STR);
    $stmt->bindParam(5, $_SESSION['uid'], PDO::PARAM_STR);
...
<body>
    ...
    <div style="height: 100vh;  width: 100%; background-color:#39ac37">
        ...
        <div style="background-color: white; width: 800px; height: 100vh; margin: 0 auto;">
            <div class="votesdiv">
                <h1>Update Poll Respond</h1>
                <h2>You can update your respond the below poll and<br>
